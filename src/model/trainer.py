...
from config import COUNTING_COLUMN, TIME_RANGE_COLUMN, MODEL_TYPE, SEASONAL_FREQ
...
def _train_seasonal(df: pd.DataFrame):
    ...
    detectors = {}
    for cat, group in df.groupby("vehicle_category"):
        ts = group.set_index(TIME_RANGE_COLUMN)[COUNTING_COLUMN].sort_index()
...
def _detect_seasonal(detectors: dict, df: pd.DataFrame):
    ...
    for cat, detector in detectors.items():
        ...
        for idx in group.index:
            ts_val = group.loc[idx, TIME_RANGE_COLUMN]
            if ts_val in anomaly_mask.index and anomaly_mask.loc[ts_val]:
